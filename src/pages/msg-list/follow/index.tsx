...
function Item(props: { item: FollowMsgItem }) {
  const { item } = props;
    ...
    <View className='flex gap-3 items-center px-4 py-2'>
      ...
      <View className="grow">
        <View className='flex justify-between items-center'>
          ...
          <Text className='c-gray-5 text-sm'>{timeStr}</Text>
        </View>
        <View className='mt-1 c-gray-5 text-sm'>关注了你</View>
      </View>
    </View>
  );
}
...
export default function Index() {
  ...
  const [list, setList] = useState<FollowMsgItem[]>([]);
  ...
  const onLoad = () => {
    setLoading(true);
    const newList: FollowMsgItem[] = refreshingRef.current ? [] : list;
    setTimeout(() => {
