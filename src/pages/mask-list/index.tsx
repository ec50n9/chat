...
function Item(props: {
  ...
  return (
    <View className='px-3 py-2 pb-0 flex justify-between gap-3 bg-white'>
      <Avatar className='shrink-0' src={props.avatar} size='medium' shape="rounded" />
      <View className='grow flex flex-col b-b-2 b-b-solid b-b-gray-3 of-hidden'>
        <View className='flex justify-between items-center'>
          <Text>{props.title}</Text>
          <Text className='text-sm c-gray-5'>{dateStr}</Text>
        </View>
        <View className='flex justify-between items-center pt-1 pb-2 of-hidden'>
          <Text className='text-sm c-gray-5 truncate'>{props.subTitle}</Text>
